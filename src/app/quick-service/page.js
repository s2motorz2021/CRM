...
export default function QuickServicePage() {
    ...
    const fetchData = async () => {
        try {
            const [custRes, vehRes, staffRes, partsRes] = await Promise.all([
                fetch('/api/customers'),
                fetch('/api/vehicles'),
                fetch('/api/staff'),
                fetch('/api/parts')
            ...
            <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 'var(--spacing-xl)', boxShadow: 'var(--shadow-sm)', minHeight: '400px' }}>
                ...
                {currentStep === 1 && (
                    <div>
                        ...
                        {/* Service Type (locked) */}
                        <div style={{ marginTop: 'var(--spacing-lg)', padding: 'var(--spacing-sm) var(--spacing-md)', background: 'rgba(76, 175, 80, 0.1)', borderRadius: 'var(--radius-md)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
                            <span>🔒</span>
                            <span style={{ fontWeight: 600, color: '#4CAF50' }}>Service Type: Quick Service</span>
                        </div>
                    </div>
                )}
